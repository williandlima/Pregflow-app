...
async function dbSet(collection, id, data) {
  if (db) await db.collection(collection).doc(id).set(data, { merge: true });
  saveLocal();
}
...
function renderHome() {
  ...
  const statsEl = document.getElementById('home-hero-stats');
  if (yearEl) yearEl.textContent = state.year;
  if (!statsEl) return;
  const all = getSundaysForYear(state.year);
  const scheduled = all.filter(d => getWeekData(d).teamId).length;
  const teamsCount = state.teams.length;
  if (scheduled > 0) {
    statsEl.textContent = `${scheduled} de ${all.length} domingos escalados · ${teamsCount} equipe${teamsCount !== 1 ? 's' : ''}`;
  } else {
    statsEl.textContent = `${all.length} domingos em ${state.year} · ${teamsCount} equipe${teamsCount !== 1 ? 's' : ''}`;
  }
}
...
function getDay(str) { return parseDate(str).getDate(); }
...
function getTeamColor(teamId) { return getTeam(teamId)?.color || '#7C3AED'; }
function getWeekData(dateStr) { return state.schedule[dateStr] || {}; }
function getWeekTeam(dateStr) { return getTeam(getWeekData(dateStr).teamId); }
...
function getMinisterForWeek(dateStr) { return getWeekMember(dateStr, 'minister'); }

async function autoGenerateSchedule() {
  if (state.teams.length === 0) { showToast('Cadastre as equipes primeiro'); return; }
  ...
  sundays.forEach((dateStr, i) => {
    ...
    if (!newSchedule[dateStr]) newSchedule[dateStr] = { teamId, overrides: {}, songs: [] };
    else newSchedule[dateStr].teamId = teamId;
  });
...
function renderMinisterScreen() {
  const container = document.getElementById('equipe-cards-list');
  const subtitle = document.getElementById('equipe-subtitle');
  if (state.teams.length === 0) {
    if (subtitle) subtitle.textContent = '';
    container.innerHTML = `<div class="empty-state" style="padding:60px 16px"><div class="empty-state-icon">👥</div><h3>Nenhuma equipe cadastrada</h3><p>Cadastre as equipes em Membros & Equipes</p></div>`;
    return;
  }
  const all = getSundaysForYear(state.year);
  if (subtitle) subtitle.textContent = `${state.teams.length} equipes · ${state.year}`;
  container.innerHTML = state.teams.map((team, idx) => {
    ...
    const members = ROLES.map(role => {
      ...
      return member ? { role, name: member.name } : null;
    }).filter(Boolean);
    const dates = all.filter(d => getWeekData(d).teamId === team.id);
    const membersHtml = members.length > 0
      ? members.map(m => `<div class="equipe-member-row"><span class="equipe-member-role">${ROLES_LABELS[m.role]}</span><span class="equipe-member-name">${m.name}</span></div>`).join('')
      : '<div class="equipe-empty">Nenhum membro atribuído</div>';
    const datesHtml = dates.length > 0
      ? dates.map(d => `<span class="equipe-date-chip" onclick="openWeekDetail('${d}')">${formatDateShort(d)}</span>`).join('')
      : '<div class="equipe-empty">Sem domingos agendados — gere a escala</div>';
    return `<div class="equipe-card"><div class="equipe-card-header" style="background:${color}"><div class="equipe-card-num">${String(idx+1).padStart(2,'0')}</div><div class="equipe-card-info"><div class="equipe-card-title">${team.name}</div><div class="equipe-card-count">${dates.length} domingo${dates.length!==1?'s':''} em ${state.year}</div></div></div><div class="equipe-card-body"><div class="equipe-section-label">👥 Membros</div><div class="equipe-members">${membersHtml}</div><div class="equipe-section-label">📅 Domingos</div><div class="equipe-dates">${datesHtml}</div></div></div>`;
  }).join('');
}
...
function openWeekDetail(dateStr) {
  ...
  rolesDiv.innerHTML = ROLES.map(role => {
    ...
    return `<div class="role-row"><span class="role-label">${ROLES_LABELS[role]}</span><span class="role-value ${name?'':'role-empty'}">${name||'Não definido'}</span></div>`;
  }).join('');
...
function renderWeekSongs(dateStr) {
  ...
  div.innerHTML = songs.length === 0
    ...
    : songs.map((s, i) => `<div class="song-chip"><span class="song-chip-title">${s.title}${s.artist?` <span style="color:var(--text-muted);font-weight:400">· ${s.artist}</span>`:''}</span><span class="song-chip-key">${s.key}</span><button class="song-chip-del" onclick="deleteSongFromWeek('${dateStr}',${i})">✕</button></div>`).join('');
}
...
async function saveSong() {
  ...
  const globalSong = { title, key, artist, date: dateStr, ministerId: getMinisterForWeek(dateStr)||'', ministerName };
...
function openEditWeekTeam() {
  ...
  const memberOptions = state.members.map(m => `<option value="${m.id}">${m.name} (${ROLES_LABELS[m.role]||m.role})</option>`).join('');
  document.getElementById('edit-week-roles').innerHTML = ROLES.map(role => `<div class="form-group"><label class="form-label">${ROLES_LABELS[role]}</label><select class="form-select" id="override-${role}"><option value="">— Usar equipe padrão —</option>${memberOptions}</select></div>`).join('');
  ROLES.forEach(role => {
...
function renderMembers() {
  ...
  list.innerHTML = filtered.map(m => `<div class="member-row"><div class="member-avatar">${m.name.charAt(0).toUpperCase()}</div><div class="member-info"><div class="member-name">${m.name}</div><div class="member-role">${ROLES_LABELS[m.role]||m.role}</div></div><div class="member-actions"><button class="icon-btn" onclick="editMember('${m.id}')" title="Editar">✏️</button><button class="icon-btn" onclick="deleteMember('${m.id}')" title="Excluir" style="color:var(--danger)">🗑️</button></div></div>`).join('');
}
...
function renderTeams() {
  ...
  if (state.teams.length === 0) {
    list.innerHTML = `<div style="padding:16px"><button class="btn btn-primary" style="width:100%" onclick="createDefaultTeams()">✨ Criar 5 Equipes Padrão</button></div>`;
    return;
  }
  const memberOptions = (selectedId) => state.members.map(m => `<option value="${m.id}" ${m.id===selectedId?'selected':''}>${m.name}</option>`).join('');
  list.innerHTML = state.teams.map((team, idx) => `
    <div class="team-card" id="team-card-${team.id}">
      ...
      <div class="team-body">
        ${ROLES.map(role => `<div class="form-group" style="padding:0 0 10px"><label class="form-label">${ROLES_LABELS[role]}</label><select class="form-select" id="team-${team.id}-${role}" onchange="updateTeamRole('${team.id}','${role}',this.value)"><option value="">— Não definido —</option>${memberOptions(team[role])}</select></div>`).join('')}
        <div style="display:flex;gap:8px;margin-top:6px">
          <input class="form-input" style="flex:1" placeholder="Nome da equipe" value="${team.name}" onchange="renameTeam('${team.id}',this.value)">
...
function toggleTeamCard(id) { document.getElementById('team-card-' + id)?.classList.toggle('expanded'); }
...
async function createDefaultTeams() {
  ...
  for (let i = 0; i < 5; i++) {
    const id = 'team_' + (i+1);
    const team = { id, name: names[i], order: i, color: TEAM_COLORS[i], minister: null, back1: null, back2: null, guitar: null, electric: null, bass: null, drums: null, keyboard: null };
...
function renderSongs() {
  ...
  list.innerHTML = filtered.map(s => `<div class="member-row"><div class="member-avatar" style="background:var(--primary-soft);color:var(--primary);font-size:20px">🎵</div><div class="member-info"><div class="member-name">${s.title}</div><div class="member-role">${s.ministerName||''}${s.date?' · '+formatDateShort(s.date):''}${s.artist?' · '+s.artist:''}</div></div><span class="week-songs-badge" style="background:var(--primary-soft);color:var(--primary)">${s.key}</span></div>`).join('');
}
...
function saveYear() {
  ...
  if (yr >= 2024 && yr <= 2035) { state.year = yr; saveLocal(); showToast('Ano salvo: ' + yr); }
}

async function saveFirebaseConfig() {
  const raw = document.getElementById('firebase-config-input').value.trim();
  try { const config = JSON.parse(raw); await initFirebase(config); }
  catch(e) { showToast('JSON inválido: ' + e.message); }
}

function exportData() {
  const data = { year: state.year, members: state.members, teams: state.teams, schedule: state.schedule, songs: state.songs };
...
function shareWeekWhatsApp() {
  ...
  const w = getWeekData(dateStr);
  const team = getTeam(w.teamId);
  const lines = [`📅 *${formatDateLong(dateStr)}*`, `🎵 ${team?team.name:'Sem equipe'}`, '',
    ...ROLES.map(role => `${ROLES_LABELS[role]}: ${getMemberName(getWeekMember(dateStr,role))}`)];
  const songs = w.songs || [];
  if (songs.length > 0) { lines.push('', '🎼 *Repertório:*'); songs.forEach(s => lines.push(`• ${s.title} (${s.key})`)); }
...
function shareWeekCalendar() {
  ...
  window.open(`https://www.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(title)}&dates=${fmt(start)}/${fmt(end)}&details=${encodeURIComponent(desc)}`, '_blank');
}
...
function printWeek() {
  ...
  win.document.write(`<!DOCTYPE html><html><head><title>Escala ${dateStr}</title><style>body{font-family:Arial,sans-serif;padding:32px;max-width:600px;margin:auto}h1{color:#7C3AED}table{width:100%;border-collapse:collapse}@media print{@page{margin:20mm}}</style></head><body><h1>🎵 Escala de Louvor</h1><h2>${formatDateLong(dateStr)}</h2><p style="color:#666">${team?team.name:''}</p><table>${rows}</table>${songs?`<h3 style="margin-top:24px">Repertório</h3><ul>${songs}</ul>`:''}<script>window.print();window.close();<\/script></body></html>`);
}
...
function showYearPicker() {
  ...
  opts.innerHTML = [yr-1,yr,yr+1,yr+2].map(y => `<button class="btn ${y===yr?'btn-primary':'btn-ghost'}" style="width:100%" onclick="selectYear(${y})">${y}</button>`).join('');
